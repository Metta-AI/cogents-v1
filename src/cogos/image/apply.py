...
def apply_image(spec: ImageSpec, repo, *, clean: bool = False) -> dict[str, int]:
    ...
    # 6. Register all collected event types (including built-in system ticks)
    if hasattr(repo, "upsert_event_type"):
        ...
        # Built-in system tick events (generated by dispatcher, not logged)
        for tick in ("system:tick:minute", "system:tick:hour"):
            all_event_names.append((tick, "dispatcher"))
        seen: set[str] = set()
