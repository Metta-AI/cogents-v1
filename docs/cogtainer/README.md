...
The cogtainer is the cogent's shared persistence and state management layer — a PostgreSQL-backed data store with async CRUD, event-driven architecture, and AWS infrastructure management.
...
```python
from cogtainer.db import Repository, MemoryRecord, MemoryScope, MemoryType
